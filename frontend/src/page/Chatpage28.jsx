...
const ChatPage = () => {
  ...
  const saveConversationToServer = async (entry, data) => {
    try {
      const token = getAuthToken();
      if (!token) return;
      const payload = {
      ...
      await axios.post(`${url}/conversations`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      // Refresh short list
      try {
        ...
        setServerHistory(res.data?.conversations || []);
      } catch (_) {}
    } catch (e) {
      ...
      reader.onload = () => {
        ...
        Papa.parse(csvText, {
          complete: (results) => {
            ...
            // Persist this conversation on server
            saveConversationToServer(newConversationEntry, results.data);
