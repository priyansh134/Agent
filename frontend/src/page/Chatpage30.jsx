...
const ChatPage = () => {
  ...
  const interruptRecognition = useRef({}); // For detecting interruptions during AI speech4
                ...
                >
                  ...
                  {serverHistory?.map((item) => (
                    <option key={item.id} value={item.id}>
                      {new Date(item.created_at).toLocaleString()} — {item.query?.slice(0, 50)}
                    </option>
                  ))}
                </select>
