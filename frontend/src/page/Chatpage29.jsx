...
const ChatPage = () => {
  ...
  const saveConversationToServer = async (entry, data, options = {}) => {
    try {
      const token = getAuthToken();
      if (!token) return;
      const payload = {
        ...
        customization: chartCustomization[entry.id] || { xAxis: 0, yAxis: 1, chartType: entry.chartType },
      ...
      if (newId) setCurrentConversationServerId(newId);
      // Refresh short list
      try {
      ...
      console.error('Failed to save conversation to server:', e);
    }
  };

  const appendMessagesToServer = async (serverConvId, messages) => {
    try {
      const token = getAuthToken();
      if (!token || !serverConvId || !messages?.length) return;
      await axios.post(`${url}/conversations/${serverConvId}/messages`, { messages }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (e) {
  ...
  const loadConversationFromServer = async (convId) => {
    try {
      ...
      setChartCustomization(prev => ({
        ...prev,
        [newEntry.id]: conv.customization || { xAxis: 0, yAxis: 1, chartType: conv.chartType || 'bar' }
      }));
      setChatConversations(prev => ({
        ...prev,
        [newEntry.id]: (conv.messages || []).map(m => ({
          type: m.type,
          message: m.message,
      ...
      showNotification('📂 Loaded conversation from your history', 'success');
    } catch (e) {
      ...
      reader.onload = () => {
        ...
        Papa.parse(csvText, {
          complete: (results) => {
            ...
            // Persist this conversation on server with initial messages
            const initialMessages = [];
            saveConversationToServer(newConversationEntry, results.data, { messages: initialMessages });
  ...
  const sendChatMessage = async (conversationId, message = null) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Append both user question and AI answer to server conversation if available
        if (currentConversationServerId) {
          await appendMessagesToServer(currentConversationServerId, [
  ...
  const newChat = () => {
    // Clear only active in-memory session (like ChatGPT new chat)
