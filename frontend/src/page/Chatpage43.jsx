...
import { BarChartIcon, LineChart, PieChart, Users, Calendar, DollarSign, Paperclip, Plus, ArrowRight, Download, ChevronDown, Send, Upload, Mic, MicOff, Brain, Volume2, VolumeX, MessageCircle, User, Bot, X } from 'lucide-react';
...
const ChatPage = () => {
  ...
  const [currentConversationServerId, setCurrentConversationServerId] = useState(null);
  const [conversationIdToQueryIdMap, setConversationIdToQueryIdMap] = useState({}); // localConvId -> server query id
  ...
  // Auth token helper
  const getAuthToken = () => {
    return localStorage.getItem('dashboardAgent_token');
  };
  ...
  // Load conversation history from server and then localStorage on component mount
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        // Fetch latest 50 from server
        const res = await axios.get(`${url}/conversations?limit=50`, {
      ...
      // Also restore local session-based history for continuity
  ...
  const createServerConversation = async ({ title, initialQuery, localConvId }) => {
    const token = getAuthToken();
    const payload = {
      title,
      initial_query: initialQuery
    };
    const res = await axios.post(`${url}/conversations`, payload, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const serverConvId = res.data?.id;
    const queryId = res.data?.query_id;
    setCurrentConversationServerId(serverConvId);
    setConversationIdToQueryIdMap(prev => ({ ...prev, [localConvId]: queryId }));
    // refresh list
    try {
      const r = await axios.get(`${url}/conversations?limit=50`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setServerHistory(r.data?.conversations || []);
    } catch (_) {}
  };

  const addServerQuery = async ({ serverConvId, query, data, chartType, customization, localConvId }) => {
    const token = getAuthToken();
    const res = await axios.post(`${url}/conversations/${serverConvId}/queries`, {
      query,
      data,
      chartType,
      customization
  ...
  const appendMessagesToServerQuery = async ({ serverConvId, queryId, messages }) => {
    const token = getAuthToken();
    if (!serverConvId || !queryId || !messages?.length) return;
    await axios.post(`${url}/conversations/${serverConvId}/queries/${queryId}/messages`, { messages }, {
      headers: { Authorization: `Bearer ${token}` }
    });
  };

  const loadConversationFromServer = async (convId) => {
    // Clear current UI to show only the selected chat session
    ...
    setContinuousMode({});
    setPinnedCharts([]);
    setConversationIdToQueryIdMap({});
    chartInstances.current = {};
    if (window.speechSynthesis) {
      window.speechSynthesis.cancel();
    }
    // Stop any in-progress interrupt detection/listening
    if (interruptRecognition.current) {
      Object.keys(interruptRecognition.current).forEach((cid) => {
        try { stopInterruptDetection(cid); } catch (e) {}
      });
    }
    if (chatRecognition.current) {
      Object.keys(chatRecognition.current).forEach((cid) => {
        try { stopVoiceListening(cid); } catch (e) {}
      });
    }
    try {
      const token = getAuthToken();
      const res = await axios.get(`${url}/conversations/${convId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const conv = res.data?.conversation;
      if (!conv) return;

      // For each query in this session, add a separate local card to the page
      let lastLocalId = null;
      const localIds = [];
      (conv.queries || []).forEach(q => {
        const newEntry = {
        ...
        localIds.push(newEntry.id);
        lastLocalId = newEntry.id;
        setConversationHistory(prev => [...prev, newEntry]);
        setChartCustomization(prev => ({
          ...prev,
          [newEntry.id]: q.customization || { xAxis: 0, yAxis: 1, chartType: q.chartType || 'bar' }
        }));
        setChatConversations(prev => ({
          ...prev,
          [newEntry.id]: (q.messages || []).map(m => ({
            type: m.type,
            message: m.message,
            timestamp: m.timestamp ? new Date(m.timestamp) : new Date()
          }))
        }));
        setShowChatInterface(prev => ({ ...prev, [newEntry.id]: true }));
        // map local card -> server query id
        setConversationIdToQueryIdMap(prev => ({ ...prev, [newEntry.id]: q.id }));
      });
      setCurrentConversationServerId(conv.id);
      if (localIds.length) {
        showNotification(`📂 Loaded ${localIds.length} queries from this chat`, 'success');
      } else {
        showNotification('📂 Loaded chat', 'success');
      }
    } catch (e) {
      console.error('Failed to load conversation:', e);
      showNotification('❌ Failed to load conversation', 'info');
    }
  };
  ...
  const generateSQL = async () => {
    try {
      ...
      if (useDatabaseMode) {
        // Require schema and connection
        if (!dbSchema) {
          showNotification('❌ Connect to database and load schema first', 'info');
          setDbModalOpen(true);
          return;
        }
        setIsLoading(true);
        ...
        setChartCustomization(prev => ({ ...prev, [newConversationEntry.id]: { xAxis: 0, yAxis: 1, chartType: selectedChartType } }));

        // Call backend to generate and execute SQL against MySQL (DB mode)
        try {
          const token = getAuthToken();
          const res = await axios.post(`${url}/generate_sql_mysql`, {
            text: queryToSend,
            ...dbConn
          ...
          reader.onload = async () => {
            ...
            return !allEmpty;
                ...
                if (!currentConversationServerId) {
                  await createServerConversation({
                    ...
                    initialQuery: {
                      ...
                      data: results.data,
                      chartType: selectedChartType,
                      customization: chartCustomization[newConversationEntry.id] || { xAxis: 0, yAxis: 1, chartType: selectedChartType },
                      messages: initialMessages
                    },
                    localConvId: newConversationEntry.id
                  });
                } else {
                  await addServerQuery({
                    serverConvId: currentConversationServerId,
                    query: queryToSend,
                    data: results.data,
                    chartType: selectedChartType,
                    customization: chartCustomization[newConversationEntry.id] || { xAxis: 0, yAxis: 1, chartType: selectedChartType },
          ...
          setConversationHistory(prev => prev.map(entry => entry.id === newConversationEntry.id ? { ...entry, isLoading: false, error: 'Failed to run DB query' } : entry));
          setIsLoading(false);
          setDisplay(false);
          return;
        }
      ...
      // CSV MODE BELOW (existing)
      if (!filePath) {
        alert("Please upload a file");
        return;
      }

      // Ensure CSV mode when using file
      if (useDatabaseMode) {
        setUseDatabaseMode(false);
      }
      ...
      setChartCustomization(prev => ({
        ...prev,
        [newConversationEntry.id]: { xAxis: 0, yAxis: 1, chartType: selectedChartType }
      }));

      const res = await axios.post(`${url}/generate_sql`, { text: queryToSend, filePath }, { responseType: 'blob' });
      ...
      setConversationHistory(prev => prev.map(entry => entry.isLoading ? { ...entry, isLoading: false, error: "Failed to generate results" } : entry));
      setIsLoading(false);
      setDisplay(false);
      setBadgeCount(0);
    }
  ...
  const generateChartOptions = (data, chartType = selectedChartType, customXAxis = null, customYAxis = null, conversationId = null) => {
    ...
    switch (selectedChartType) {
      ...
      case 'area':
        return {
          title: {
            text: `${headers[yAxisIndex]} vs ${headers[xAxisIndex]} (Area)`,
            left: 'center',
            textStyle: {
              fontSize: 18,
              fontWeight: 'bold',
      ...
      case 'horizontalBar':
        return {
          title: {
            text: `${headers[yAxisIndex]} by ${headers[xAxisIndex]} (Horizontal)`,
            left: 'center',
          ...
          tooltip: { trigger: 'axis' },
          grid: { left: '20%', right: '10%', bottom: '10%', top: '15%', containLabel: true }
        };
      case 'donut':
        return {
          title: {
            text: `Donut of ${headers[yAxisIndex]}`,
            left: 'center',
            textStyle: { fontSize: 18, fontWeight: 'bold', color: '#2c3e50' }
          },
          series: [{
      ...
      case 'funnel':
        return {
          title: {
            text: `${headers[yAxisIndex]} Funnel by ${headers[xAxisIndex]}`,
            left: 'center',
            textStyle: { fontSize: 18, fontWeight: 'bold', color: '#2c3e50' }
          },
          series: [{
            ...
            data: seriesData.map((d, idx) => ({ name: d.name, value: d.value, itemStyle: { color: colors[idx % colors.length] } }))
          }],
  ...
  const sendChatMessage = async (conversationId, message = null) => {
    ...
    // Find the query card for this conversationId
    const conversation = conversationHistory.find(conv => conv.id === conversationId);
    if (!conversation) return;

    setIsChatting(prev => ({ ...prev, [conversationId]: true }));

    const newChatMessage = { type: 'user', message: userQuestion, timestamp: new Date() };
    setChatConversations(prev => ({ ...prev, [conversationId]: [...(prev[conversationId] || []), newChatMessage] }));
    ...
    try {
      ...
      if (response.data.success) {
        const aiResponse = { type: 'ai', message: response.data.answer, timestamp: new Date() };
        setChatConversations(prev => ({ ...prev, [conversationId]: [...(prev[conversationId] || []), newChatMessage, aiResponse] }));

        // Append messages under the correct server query id within the session
        const serverConvId = currentConversationServerId;
        const queryId = conversationIdToQueryIdMap[conversationId];
        if (serverConvId && queryId) {
          await appendMessagesToServerQuery({
            serverConvId,
            queryId,
            messages: [
              { type: 'user', message: newChatMessage.message, timestamp: newChatMessage.timestamp.toISOString() },
              { type: 'ai', message: aiResponse.message, timestamp: aiResponse.timestamp.toISOString() }
            ]
          });
        }
  ...
  const newChat = () => {
    ...
    setShowChatInterface({});
    setContinuousMode({});
    setCurrentConversationServerId(null);
    setConversationIdToQueryIdMap({});
    window.speechSynthesis?.cancel();
    showNotification('✨ Started a new chat', 'success');
  };

  const openDbModal = () => setDbModalOpen(true);
  const closeDbModal = () => setDbModalOpen(false);

  const fetchDbSchema = async () => {
    setIsFetchingSchema(true);
    try {
      const token = getAuthToken();
      const res = await axios.post(`${url}/db/schema`, dbConn, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setDbSchema(res.data.schema);
      setIsConnectedToDatabase(true);
      setUseDatabaseMode(true);
      showNotification('✅ Connected and schema loaded. Using Database mode for queries.', 'success');
    } catch (e) {
      console.error('Schema fetch failed:', e);
      setIsConnectedToDatabase(false);
      showNotification('❌ Failed to connect or load schema', 'info');
    } finally {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        {dbModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg border">
              <div className="flex items-center justify-between p-4 border-b">
                <h3 className="text-lg font-semibold">Connect to MySQL Database</h3>
                <button onClick={closeDbModal} className="p-1 hover:bg-gray-100 rounded">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="w-full border rounded px-2 py-1 text-sm" value={dbConn.host} onChange={e => setDbConn({ ...dbConn, host: e.target.value })} placeholder="127.0.0.1" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600">Port</label>
                    <input className="w-full border rounded px-2 py-1 text-sm" value={dbConn.port} onChange={e => setDbConn({ ...dbConn, port: e.target.value })} placeholder="3306" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600">User</label>
                    <input className="w-full border rounded px-2 py-1 text-sm" value={dbConn.user} onChange={e => setDbConn({ ...dbConn, user: e.target.value })} placeholder="root" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600">Password</label>
                    <input type="password" className="w-full border rounded px-2 py-1 text-sm" value={dbConn.password} onChange={e => setDbConn({ ...dbConn, password: e.target.value })} placeholder="••••••" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600">Database</label>
                    <input className="w-full border rounded px-2 py-1 text-sm" value={dbConn.database} onChange={e => setDbConn({ ...dbConn, database: e.target.value })} placeholder="dbname" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600">Table</label>
                    <input className="w-full border rounded px-2 py-1 text-sm" value={dbConn.table} onChange={e => setDbConn({ ...dbConn, table: e.target.value })} placeholder="table_name" />
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <label className="flex items-center text-sm space-x-2">
                    <input type="checkbox" checked={useDatabaseMode} onChange={e => setUseDatabaseMode(e.target.checked)} />
                    <span>Use Database Mode (instead of CSV)</span>
                  </label>
                  <button onClick={fetchDbSchema} disabled={isFetchingSchema} className={`px-3 py-1.5 rounded-lg text-white text-sm ${isFetchingSchema ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}>
                    {isFetchingSchema ? 'Loading...' : 'Load Schema'}
                  </button>
                </div>
                {dbSchema && (
                  <div className="mt-2 p-2 bg-gray-50 border rounded text-xs text-gray-700 max-h-40 overflow-auto">
                    <div className="font-medium mb-1">Schema loaded for `{dbSchema.table_name}`</div>
                    {dbSchema.columns.slice(0, 8).map((c, idx) => (
                      <div key={idx}>- {c.name} ({c.type})</div>
                    ))}
                    {dbSchema.columns.length > 8 && <div>… {dbSchema.columns.length - 8} more columns</div>}
                  </div>
                )}
                <div className="flex justify-end space-x-2 p-2 border-t">
                  <button onClick={closeDbModal} className="px-3 py-1.5 rounded border text-sm">Close</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row h-auto lg:h-[calc(100vh-64px)]">
          ...
          >
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-1">Data Assistant</h3>
            ...
            {/* History Dropdown (from server) */}
            <div className="p-4 border-b border-gray-100" ref={historyDropdownRef}>
              <label className="block text-sm font-medium text-gray-700 mb-2">Your History</label>
              <div className="relative">
                <button
                  onClick={() => setHistoryOpen(prev => !prev)}
                  className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm text-left flex items-center justify-between hover:border-blue-400"
                >
                  <span>{isLoadingHistory ? 'Loading history...' : (serverHistory?.length ? 'Open a previous conversation...' : 'No saved history')}</span>
                  <ChevronDown className={`h-4 w-4 text-gray-400 transition-transform ${historyOpen ? 'rotate-180' : ''}`} />
                </button>
                {historyOpen && serverHistory?.length > 0 && (
                  <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto custom-scrollbar">
                    {serverHistory.map((item) => (
                      <button
                        key={item.id}
                        onClick={() => { setHistoryOpen(false); loadConversationFromServer(item.id); }}
              ...
              {isConnectedToDatabase ? (
                <p className="text-xs text-green-600 mt-1">✅ Synced with cloud history</p>
              ) : (
                <p className="text-xs text-gray-500 mt-1">Offline: showing local history only</p>
              )}
            </div>
