    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                # Prefer INFORMATION_SCHEMA for robust schema
                cur.execute(
                    """
                ...
                for col_name, data_type, is_nullable in cols_info:
                    columns.append({
                        ...
                        "null_count": None,
                        "unique_values": None,
                        "data_category": "numeric" if data_type in ("int","bigint","tinyint","smallint","mediumint","decimal","float","double") else ("datetime" if data_type in ("date","datetime","timestamp","time","year") else "text/categorical")
                    })
                # Attempt small sample to enrich stats
                try:
                    cur.execute(f"SELECT * FROM `{payload['database']}`.`{payload['table']}` LIMIT 200")
                    sample_rows = cur.fetchall()
                    headers = [desc[0] for desc in cur.description]
                except Exception as inner_e:
                    # Sampling failed; continue with column-only schema
                    sample_rows = []
                    headers = [c[0] for c in cols_info] if cols_info else []
        finally:
            conn.close()
        # Build DataFrame for stats if we have any
...
def generate_sql_mysql(current_user_id):
    ...
    try:
        # Fetch small sample to build schema
        conn = get_mysql_connection(data)
        try:
            with conn.cursor() as cur:
                cur.execute(f"SELECT * FROM `{data['table']}` LIMIT 200")
                rows = cur.fetchall()
                headers = [desc[0] for desc in cur.description]
        finally:
            conn.close()
    ...
    try:
        ...
        return jsonify({"error": f"Error executing SQL: {str(e)}", "generated_query": sql_query}), 500
