...
def mysql_schema(current_user_id):
	...
	try:
		...
		schema_info = json_safe(schema_info)
...
def get_schema_info(df):
	...
	schema_info = {
		...
		"sample_data": json_safe(df.head(3).to_dict('records'))
	}
	...
	for col in df.columns:
		...
		if df[col].dtype in ['object', 'string']:
			...
			col_info["max_value"] = json_safe(df[col].max())
			mean_val = df[col].mean()
			col_info["mean_value"] = round(float(mean_val), 2) if pd.notna(mean_val) else None
			col_info["data_category"] = "numeric"
		elif str(df[col].dtype) in ['datetime64[ns]', 'datetime']:
		...
		schema_info["columns"].append(json_safe(col_info))
	
	return schema_info


def json_safe(obj):
	"""Recursively convert numpy/pandas/Decimal/datetime/bytes to JSON-safe Python types."""
